...
static void* sdio_malloc_internal(size_t size, hosted_mem_cap_t caps) {
	return heap_caps_malloc(size, MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
}
static void* sdio_calloc_internal(size_t n, size_t size, hosted_mem_cap_t caps) {
	void* p = heap_caps_malloc(n * size, MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
...
static inline void sdio_mempool_create(int tx_q_size, int rx_q_size)
{
	...
	hosted_mempool_config_t config = {
		...
		.malloc = sdio_malloc_internal,
		.calloc = sdio_calloc_internal,
		.memset = g_h.funcs->_h_memset,
		.free   = g_h.funcs->_h_free,
	};
	buf_mp_g = hosted_mempool_create(&config);
	if (!buf_mp_g) {
    ESP_LOGW(TAG, "mempool create failed, using direct alloc");
}
#endif
}
...
static inline void *sdio_buffer_alloc(uint need_memset)
{
    ...
    /* SDIO DMA requires internal memory, not SPIRAM */
    void *p = heap_caps_malloc(MAX_SDIO_BUFFER_SIZE, MALLOC_CAP_DMA | MALLOC_CAP_INTERNAL);
...
static esp_err_t sdio_push_data_to_queue(uint8_t * buf, uint32_t buf_len)
{
	...
	do {
		...
		if (!pkt_rxbuff) {
    ESP_LOGE(TAG, "sdio_buffer_alloc failed, skip packet");
    continue;
}
